<template>
  <div class="third-content">
    ...
    <div class="content-middle">
      <div id="chart-revenue-1" style="flex: 1; height: 300px" />
      <div id="chart-revenue-2" style="flex: 1; height: 300px" />
    </div>
...
const initChart1 = () => {
  if (!chart.value) {
    chart.value = echarts.init(
      document.getElementById("chart-revenue-1") as HTMLDivElement
    );
  }
  chart.value.clear();
  ...
  const option = {
    ...
    legend: {
      orient: "vertical",
      left: "left",
      data: revenueData.value.map((item) => item.企业名称),
      textStyle: {
        color: "#fff",
      },
    },
    series: [
      {
        ...
        radius: "55%",
        center: ["50%", "60%"],
        data: revenueData.value.map((item) => ({
          value: item.累计营收,
          name: item.企业名称,
        })),
        label: {
...
const initChart2 = () => {
  if (!chart2.value) {
    chart2.value = echarts.init(
      document.getElementById("chart-revenue-2") as HTMLDivElement
    );
  }
  chart2.value.clear();
    ...
    series: [
      {
        ...
        barWidth: "40%",
        barGap: "10%", // 柱体间距
        data: revenueData.value.map((item) => item.累计营收),
        name: "累计营收",
        // 显示标签
        label: {
          show: true,
      ...
      },
      {
        ...
        barWidth: "40%",
        barGap: "10%", // 柱体间距
        data: revenueData.value.map((item) => item.目标营收),
        name: "目标营收",
        // 显示标签
        label: {
          show: true,
