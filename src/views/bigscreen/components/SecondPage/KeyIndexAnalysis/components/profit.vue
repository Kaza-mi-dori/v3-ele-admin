...
const buildQueryParams = () => {
  ...
  const queryParams = {
    ...
    fromMonth: 1,
    toMonth: 12,
    category: route.query.productName || undefined,
    product: undefined,
...
const initData = async () => {
  ...
  const testData: AggregatedData[] = await businessStore.queryKeyIndexData(
    queryParams.keyIndexType as "利润" | "利润",
  ...
  if (testData?.[0]) {
    data = {
      ...
      chart3: {
        data: fulfillArray(testData[0].subOrgData).map((item: any) => {
          return {
            name: item.维度值,
            value: item.实际值,
          };
...
.sub-org-table {
  ...
  tbody {
    @apply w-full overflow-y-auto; /* 设置高度，留出thead的高度 */
    // 超出滚动条时，隐藏滚动条
    // &::-webkit-scrollbar {
    //   display: none;
    // }
