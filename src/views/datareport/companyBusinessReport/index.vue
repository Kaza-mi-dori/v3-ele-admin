<template>
  <div class="main-wrapper">
    ...
    >
      ...
      <!-- 聚合标签 -->
      <el-table-column prop="name" label="类型" sortable align="center">
        <template v-slot="scope">
          <span>{{ scope.row.聚合标签 || "-" }}</span>
        </template>
      </el-table-column>
      <!-- 聚合方式 -->
      <!-- <el-table-column prop="name" label="聚合方式" sortable align="center">
        <template v-slot="scope">
          <span>{{ scope.row.聚合方式 || "-" }}</span>
        </template>
      </el-table-column> -->
      <!-- 数据日期 -->
      ...
      <el-form :model="exportForm" label-width="120px" label-position="top">
        ...
        <el-form-item label="数据时间">
          <el-date-picker
            ...
            :type="exportForm.type === '企业年报' ? 'year' : 'month'"
            :value-format="exportForm.type === '企业年报' ? 'YYYY' : 'YYYY-MM'"
          />
        </el-form-item>
      </el-form>
      <template #footer>
...
const uploadUrl =
  ...
  "/Api/Business/ProductMonthReport/Import/importExcel";
const uploadHeaders = {
...
const exportConfigDialogRef =
  ref<Nullable<InstanceType<typeof ElDialog>>>(null);
...
const isDisabledExport = computed(() => {
  return !exportForm.value.type || !exportForm.value.enterprise;
});
...
const handleDelete = (row: any) => {
  ...
  handleDeleteRow(
    row,
    BusinessFormAPI.deleteMonthlyProductReportFormByIds,
    () => {
      initTableData();
    }
  );
...
const handleAudit = async (row: any) => {
  // 先获取详情数据
  await BusinessFormAPI.getMonthlyProductReportForm(row.id).then((res) => {
    // console.log(res);
    handleAuditRow(
      row,
      BusinessFormAPI.editMonthlyProductReportForm,
      "状态",
      "有效",
      () => {
        initTableData();
...
const handleConfirmFilter = (value: any) => {
  queryForm.value = {
    ...
    日期晚于: value["时间跨度"]?.[0],
    日期早于: value["时间跨度"]?.[1],
    聚合标签: value["聚合标签"] ? [value["聚合标签"]] : undefined,
  };
  initTableData();
};
...
const initTableData = async () => {
  loading.value = true;
  try {
    const res: any = await BusinessFormAPI.getMonthlyProductReportFormList({
      ...queryForm.value,
...
const handleConfirmExportConfig = async () => {
  ...
  const types = ["企业年报", "企业月报"];
  const query = {
    数据日期: getDateString(new Date(exportForm.value.dataTime)),
    企业名称: exportForm.value.enterprise,
    聚合标签: types.filter((type) => exportForm.value.type === type),
  };
...
const onExportExcel = async (query: any) => {
...
const handleExportExcel = async (query: any) => {
  // console.log("导出excel");
  try {
    const res = await BusinessFormAPI.exportMonthlyProductReportForm({
      ...query,
    });
    const fileName = decodeURI(
      `企业经营报表导出(${query.企业名称})` +
        getDateString(new Date(query.数据日期)) +
        ".xlsx"
    );
    const fileType =
 ...
 * 提交多个数据
 **/
const handleSubmitDataInBatch = async (data: any) => {
  // console.log(data);
  loading.value = true;
  try {
    await BusinessFormAPI.addOrUpdateMonthlyProductReportForm(data);
...
const handleGetExcelTemplate = () => {
  BusinessFormAPI.getMonthlyProductReportImportTemplate().then((res: any) => {
    const fileName = "产品经营报表导入模板.xlsx";
    const fileType =
      "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8";
    const blob = new Blob([res.data], {
      type: fileType,
