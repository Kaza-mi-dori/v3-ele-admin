        ...
        >
          ...
          <el-table-column prop="正文内容" label="正文内容" align="center">
            <template #default="{ row }">
              <el-text
                truncated
                class="!mx-2 flex-1 !text-[var(--el-text-color-secondary)]"
              >
                {{ row.正文内容 || "-" }}
              </el-text>
            </template>
...
} from "@/api/businessStandBook/dingding/setttlement";
...
const {
  ...
  fetchListApi: queryDingTalkSettlementLedger,
  addApi: addDingTalkSettlementLedger,
  deleteApi: deleteDingTalkSettlementLedger,
  defaultQuery: {
    页码: 1,
    页容量: 10,
  },
...
const handleSync = (startDate: string, endDate: string) => {
  ...
  syncDingTalkSettlementLedger(startDate, endDate).then((res) => {
    console.log(res);
  });
};
