...
const initValue = async (newVal: string | undefined) => {
  ...
  try {
    const res: any = await BusinessStandbookAPI.getOrderLedgerRecordList({
...
const queryOrderSearch = async (query: string) => {
  const res: any = await BusinessStandbookAPI.getOrderLedgerRecordList({
    [props.propQueryInList ? props.prop + "集合" : props.prop]:
...
const debouncedSearch = useDebounceFn(queryOrderSearch, 800);
</script>
