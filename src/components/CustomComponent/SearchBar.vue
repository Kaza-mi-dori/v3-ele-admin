...
.active-filters-block {
  @apply flex;
  & > .__item {
    @apply flex inline-block bg-white p-1 m-1 border border-gray-200 rounded items-center;
  }
}
.closable-tag {
...
.condition-block {
  @apply p-1 bg-white border-t-1 border-dashed flex justify-between items-center;
  // 第一个元素的上边框不显示
  &:first-child {
    @apply border-t-0;
  }
  ...
  .__item {
    @apply inline-block cursor-pointer p-1 m-1 border border-gray-200 rounded;
    // 单数行则为白底，双数行则为灰底
    &:hover {
